...
6. **Switch conversation store from file → Redis (deferred)**. Reason: Redis Cloud deleted the free DB on/around 2026-04-21 for inactivity (email confirmation, ignored at the time). When QA opened `/chat` on Render today, `/chat/send` and `/chat/reset` 500'd because `aioredis.from_url(...)` is lazy — bootstrap succeeded but the first Redis command failed. Dropped `REDIS_URL` on Render's dashboard; bootstrap fell back to `FileConversationRepository`. Acceptable for QA short sessions (state wipes on Render free-tier sleep / redeploy / push). For real prod or longer QA sessions, provision a new Redis — **Upstash recommended over Redis Cloud** (no inactivity-deletion policy, 10k commands/day free). Switch is a one-step env-var change on Render — no code change required (bootstrap.py:48–52 picks repo by env).

### Bonus shipped this session (post `/session-end`)
- **`pip install .` was silently dropping `chat.html`** — package-data declaration was missing from `pyproject.toml`. `/chat` 500'd on Render with `FileNotFoundError`. Latent bug since the chat UI shipped in Session 3 — never noticed because all browser testing was local (`uvicorn --reload` reads the file straight from the source tree). Fixed with `[tool.setuptools.package-data] call_agent = ["api/static/*"]`. Wheel verified to contain the bundle before push. Commit `745f376`.
